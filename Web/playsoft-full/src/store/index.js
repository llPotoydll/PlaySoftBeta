...
export default new Vuex.Store({
    ...
    actions: {
        getPlaylistsAction({ commit, state }) {
            ...
            axios.get(`https://playsoft-api.azurewebsites.net/users/${IdUser}`)
        ...
        getSongs({ commit, state }) {
            state.Songs = []
            state.SongsLines = []
            axios.get(`https://playsoft-api.azurewebsites.net/playlists/${state.PlayListsID}?orderKey=songName&order=asc`)
                .then(function(response) {
                    ...
                    for (let index = 0; index < songs.length; index++) {
                        var SongsLines = []
                        axios.get(`https://playsoft-api.azurewebsites.net/songs/${songs[index].song.songID}`)
        ...
        postSong({ state }) {
            axios.post('https://playsoft-api.azurewebsites.net/songs', {
        ...
        addPlaylist({ commit, dispatch, state }) {
            if (state.playListName != "") {
                ...
                axios.post("https://playsoft-api.azurewebsites.net/playlists", {
